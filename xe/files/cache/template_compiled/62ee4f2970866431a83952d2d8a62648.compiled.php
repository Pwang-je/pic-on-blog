...
.board_write{border-top: 2px solid #ccc;border-bottom: 2px solid #ccc;}
</style>
<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/ena_board_set_simpledashed','_header.html') ?>
<?php if($__Context->module_info->default_style=='memo' || $__Context->module_info->default_style=='1line_memo' || $__Context->module_info->default_style=='guestbook'){;
$__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/ena_board_set_simpledashed','write_form_simple.html');
} ?>
<?php if($__Context->module_info->default_style!='memo' && $__Context->module_info->default_style!='1line_memo' && $__Context->module_info->default_style!='guestbook'){ ?>
	...
	<div class="write_footer">
		...
		<div class="write_author">
			<?php if(!$__Context->is_logged){ ?><span class="item">
				...
				<input placeholder="<?php echo $__Context->lang->writer ?>" type="text" name="nick_name" id="userName" class="iText userName" value="<?php echo htmlspecialchars($__Context->oDocument->get('nick_name')) ?>" />
			</span><?php } ?>
			<?php if(!$__Context->is_logged){ ?><span class="item">
			...
			<?php if(!$__Context->is_logged){ ?><span class="item">
				...
				<input placeholder="<?php echo $__Context->lang->homepage ?>" type="text" name="homepage" id="homePage" class="iText homePage" value="<?php echo htmlspecialchars($__Context->oDocument->get('homepage')) ?>" />
			</span><?php } ?>
			<p class="item">
				<label for="tags" class="iLabel"><?php echo $__Context->lang->tag ?>: <?php echo $__Context->lang->about_tag ?></label>
				<input placeholder="<?php echo $__Context->lang->tag ?>: <?php echo $__Context->lang->about_tag ?>" type="text" name="tags" id="tags" value="<?php echo htmlspecialchars($__Context->oDocument->get('tags')) ?>" class="iText" title="Tag" />
			</p>		
		</div>
		<div class="btnArea">
			<input type="submit" value="<?php echo $__Context->lang->cmd_registration ?>" />
			<?php if(!$__Context->oDocument->isExists() || $__Context->oDocument->get('status') == 'TEMP'){ ?>
			<?php if($__Context->is_logged){ ?><button  type="button" onclick="doDocumentSave(this);"><?php echo $__Context->lang->cmd_temp_save ?></button><?php } ?>
			<?php if($__Context->is_logged){ ?><button  type="button" onclick="doDocumentLoad(this);"><?php echo $__Context->lang->cmd_load ?></button><?php } ?>
			<?php } ?>
		</div>
	</div>
</form>
